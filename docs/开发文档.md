...
> 面向**使用者**的说明书在仓库首页 [README](../README.md)；本文件面向开发者：构建、部署、验证、目录结构。
...
### 云托管：Cloudflare Pages 为主，WorkBuddy 发布通道为备

**主地址：https://doujin-pos-offline.pages.dev**
**备用地址：https://doujin-pos-52877.app.workbuddy.host**

发布目录由 `node scripts/build-publish.mjs` 生成（默认 `../doujin-pos-live`），
`_headers` 与 `vercel.json` 已随目录一并产出，**不需要云端构建步骤**。

#### 主：Cloudflare Pages（wrangler）

项目名 `doujin-pos-offline`。首次需登录一次（`npx wrangler login`，OAuth，浏览器点允许），之后：

```bash
node scripts/build-publish.mjs
npx wrangler pages deploy ../doujin-pos-live \
  --project-name=doujin-pos-offline --branch=main --commit-dirty=true
```

项目名与仓库名保持一致。注意 **Pages 项目不支持改名**，只能新建项目再切过去，
所以名字一次定好（`wrangler pages project create / delete`）。

部署后**必须验证**，别只看 "Deployment complete"：

```bash
# 1) 三个响应头是否生效（这条不过，应用根本起不来）
curl -sI https://doujin-pos-offline.pages.dev/ | grep -i cross-origin
# 2) 浏览器 Console 里执行，必须为 true
crossOriginIsolated
```

#### 备：WorkBuddy 发布通道

同一个发布目录，走 WorkBuddy 的应用发布即可。它自带 `server.mjs` 提供响应头，
不依赖 `_headers`。注意重新发布时**优先用「更新已有应用」**（保住原链接）；
若提示"工作区没有可更新的已有应用"，就只能新建应用、链接会变。

#### 通用注意

- **不要用 GitHub Pages** —— 不支持自定义响应头，应用会直接拒绝启动。GitLab Pages、Surge.sh 同理。
- 用 Git 集成做自动部署也可以（Build command 留空、Build output directory 填 `doujin-pos-live`），
  但该目录是构建产物、通常不入库，因此更推荐上面的直接上传/CLI 方式。
- 缓存策略已写在 `_headers` 里：`/assets/*` 长缓存，`index.html` 与 `sw.js` 不缓存。
- Service Worker 是 prompt 模式，发布后用户需在应用内确认更新。
- **换域名 = 换 origin**：OPFS 与 localStorage 按 origin 隔离，新地址不会继承旧地址的营业数据，
  需要用「导出 SQLite → 导入」迁移。
