...
- 用 Git 集成做自动部署也可以（Build command 留空、Build output directory 填 `doujin-pos-live`），
  但该目录是构建产物、通常不入库，因此更推荐上面的直接上传。
- 更新：本地重新 `build-publish.mjs` 后再次上传。Service Worker 是 prompt 模式，用户需在应用内确认更新。
